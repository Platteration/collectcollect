 ...
 * page rather than in a console nobody has open: an app that fails silently
 * is one whose owner assumes their record is gone.
 */
export function ErrorPage({ thing, error, reset }: { thing: string; error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => {
    console.error(error);
  }, [error]);
  ...
  return (
    <div className="mx-auto mt-16 max-w-md text-center">
      ...
      <p className="mt-2 text-sm" style={{ color: "var(--muted)" }}>
        This page could not be drawn. Your {thing} is unaffected: nothing here writes to it.
      </p>
