...
/**
 * Browser chrome follows the theme that is actually showing. The static meta
 * tags cover the first paint; once a preference is pinned, only the resolved
 * theme is right, so the tag is narrowed to a single colour. The two colours
 * are the app's own ground, which is why they are passed in.
 */
function setThemeColor(colors: { light: string; dark: string }) {
  const dark = document.documentElement.getAttribute("data-theme") === "dark";
  for (const tag of document.querySelectorAll('meta[name="theme-color"]')) {
    tag.setAttribute("content", dark ? colors.dark : colors.light);
