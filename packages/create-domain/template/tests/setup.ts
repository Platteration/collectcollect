 ...
 * engine mirrors each item to a Markdown file under __PREFIX___DATA_DIR, and a
 ...
 */
const root = fs.mkdtempSync(path.join(os.tmpdir(), "collectcollect-__ID__-test-"));
process.env.__PREFIX___DATA_DIR = path.join(root, "run-0");

let n = 0;
beforeEach(() => {
  n += 1;
  process.env.__PREFIX___DATA_DIR = path.join(root, `run-${n}`);
