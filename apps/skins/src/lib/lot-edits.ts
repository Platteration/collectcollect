...
import { refreshMirror } from "./items";
...
export function correctLot(itemId: number, lotId: number, patch: LotEditInput): Acquisition {
  const db = getDb();
  const lot = db.transaction(() => {
    const current = getLot(lotId);
    if (!current || current.itemId !== itemId) throw new LotEditError("Purchase not found", 404);
