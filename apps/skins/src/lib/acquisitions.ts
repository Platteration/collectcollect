 ...
 *
 * This is the card app's ledger with `card_id` renamed. It is a copy rather
 * than a shared module on purpose: the table and column names live inside raw
 * prepared statements, where a type parameter cannot reach, and templating them
 * into the SQL would make both copies harder to read than keeping two.
 *
 * A null `unitCost` is a real state, not a missing value: plenty of items
 * arrive from a case opening, a trade or a gift. Those copies are counted
 * separately rather than being priced at zero, which would read as pure profit.
 *
 * For a weapon, knife or glove there is only ever one lot of one copy, because
 * the object is unique. The ledger still earns its place there: it is what
 * remembers the cost after the item is sold. For cases and stickers, which
 * genuinely stack across years and prices, it is the whole point.
 */
...
export function listLots(itemId: number): Acquisition[] {
  return (
    getDb()
      .prepare("SELECT * FROM acquisitions WHERE item_id = ? ORDER BY acquired_at, id")
      .all(itemId) as LotRow[]
  ).map(rowToLot);
}
...
export function addLot(itemId: number, input: AcquisitionInput = {}): Acquisition {
 ...
 */
export function consumeFifo(itemId: number, quantity: number): { lots: Consumed[]; unitCost: number | null } {
  const db = getDb();
  const lots: Consumed[] = [];
  let left = quantity;
 ...
 */
export function restoreForSale(itemId: number, saleId: number): void {
  const db = getDb();
 ...
 */
export function reconcileToQuantity(itemId: number, quantity: number): void {
  const db = getDb();
...
export function costBasis(itemId: number): CostBasis {
...
export function costBasisByItem(): Map<number, CostBasis> {
  const rows = getDb()
    .prepare("SELECT * FROM acquisitions WHERE remaining > 0 ORDER BY item_id, acquired_at, id")
    .all() as LotRow[];
...
/**
 * Keep `items.purchase_price` as the average paid per copy still held. It is a
 * derived figure rather than something the item owns, but the CSV columns and
 * the Markdown front matter still read it.
 */
export function recomputePurchasePrice(itemId: number): void {
  const lots = listLots(itemId);
...
/**
 * Make the item's quantity follow its lots. Used after a sale or an undo, where
 ...
 */
export function syncQuantityFromLots(itemId: number): number {
