...
const globalForImports = globalThis as unknown as { __collectcollectImports?: Map<string, number> };
function recentlyApplied(): Map<string, number> {
  return (globalForImports.__collectcollectImports ??= new Map());
}
...
export async function POST(request: Request) {
  ...
  let body: { csv?: unknown; game?: unknown; apply?: unknown; token?: unknown };
  try {
