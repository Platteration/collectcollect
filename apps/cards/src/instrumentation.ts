export async function register() {
  if (process.env.NEXT_RUNTIME === "nodejs") {
    ...
    if (databaseExists()) {
      ...
      if (broken.length > 0) {
        console.warn(
          `[collectcollect] ${broken.length} card(s) whose quantity and purchase lots disagree: ${broken.slice(0, 20).join(", ")}${broken.length > 20 ? "…" : ""}. Editing the count on each puts them right.`,
        );
      }
    }
