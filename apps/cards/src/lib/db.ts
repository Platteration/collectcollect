...
export { DB_FILE, dataDir, resetDataDirLookup, uploadsDir };
...
export function getDb(): Database.Database {
  ...
  if (!globalForDb.__collectcollectDb) {
    const recovery = recoverCollectionSwap({ dataDir: dataDir(), databaseFile: databaseFile(), uploadsDir: path.join(dataDir(), "uploads"), collectionDir: path.join(dataDir(), "collection") });
...
/**
 * Whether the collection answers a query, for the health check. "Exists" is
 ...
 * not exist, so an install with no collection yet answers false quietly;
...
export function describeRecoveryConflicts(conflicts: RecoveryConflict[]): string {
  ...
  return `[collectcollect] An interrupted restore left two copies in place: ${pairs}. Both were kept; move the one you do not want out of the data directory. Details are on the Settings page.`;
}
