 ...
 * engine mirrors each item to a Markdown file under RETRO_GAMES_DATA_DIR, and a
 ...
 */
const root = fs.mkdtempSync(path.join(os.tmpdir(), "collectcollect-retro-games-test-"));
process.env.RETRO_GAMES_DATA_DIR = path.join(root, "run-0");

let n = 0;
beforeEach(() => {
  n += 1;
  process.env.RETRO_GAMES_DATA_DIR = path.join(root, `run-${n}`);
